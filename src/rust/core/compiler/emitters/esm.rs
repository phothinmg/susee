...
pub fn emit_esm(
    ...
    let source_type = detect_source_type(source_code).unwrap();
...
mod tests {
    ...
    fn transforms_tsx_jsx() {
        ...
        let out = emit_esm(src, Some("test.tsx".to_string()));
        assert!(
            !out.contains("<div>"),
    ...
    fn accepts_file_path() {
        ...
        let out = emit_esm(src,  Some("src/mod.ts".to_string()));
    ...
    fn returns_string_type() {
        let out = emit_esm("export const x = 1;",  None);
