...
pub struct Compiler {
    ...
    bundled_cache: std::collections::HashMap<String, BundleEntryResult>,
}
...
impl Compiler {
    ...
    fn bundle_entry(&mut self, point: &BuildEntryPoint) -> std::io::Result<BundleEntryResult> {
        ...
        self.bundled_cache
            .insert(point.entry.clone(), result.clone());
        Ok(result)
    }
    ...
    fn compile_format(
        ...
        // 1. Bundle.
        let bundle_entry_result = self.bundle_entry(point)?;
        let bundled_code = bundle_entry_result.code;
