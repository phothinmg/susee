...
import type { DepFileObject, DuplicatesNameMap, NamesSets } from "susee-types";
...
function resetDuplicateState(namesMap: DuplicatesNameMap) {
	namesMap.clear();
...
const duplicateCallExpression = (compilerOptions: ts.CompilerOptions) => {
...
const duplicateExportExpression = (compilerOptions: ts.CompilerOptions) => {
	return ({ fileName, sourceCode, sourceFile }: DepFileObject) => {
		const transformer: ts.TransformerFactory<ts.SourceFile> = (context) => {
			const { factory } = context;
			const visitor = (node: ts.Node): ts.Node => {
...
const duplicateImportExpression = (compilerOptions: ts.CompilerOptions) => {
	return ({ fileName, sourceCode, sourceFile }: DepFileObject) => {
		const transformer: ts.TransformerFactory<ts.SourceFile> = (context) => {
			const { factory } = context;
			const visitor = (node: ts.Node): ts.Node => {
...
const duplicateCollector = (compilerOptions: ts.CompilerOptions) => {
	return ({ fileName, sourceCode, sourceFile }: DepFileObject) => {
		const transformer: ts.TransformerFactory<ts.SourceFile> = (context) => {
			//const { factory } = context;
...
const duplicateUpdater = (compilerOptions: ts.CompilerOptions) => {
	return ({ fileName, sourceCode, sourceFile }: DepFileObject) => {
		const transformer: ts.TransformerFactory<ts.SourceFile> = (context) => {
			const { factory } = context;
			const visitor = (node: ts.Node): ts.Node => {
