#![doc = "Logging utilities for the SUSEE build system."]
use colored::*;
use std::time::Instant;

/// Prints an error message with the given info and cause.
///
/// If `e` is `true`, the process exits with status code `1` after printing.
...
/// Prints the elapsed time since `start` as a "Bundled" timing log.
#[allow(unused)]
...
/// Prints the elapsed time since `start` as a "Build" timing log.
