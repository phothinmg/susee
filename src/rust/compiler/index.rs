...
/// `types` fields from the collected [`OutFiles`].
fn write_package_json(files: &OutFiles, export_path: &str) -> std::io::Result<()> {
    ...
    // Force ESM-by-default for the package, matching the TS port.
    ...
    if is_main {
        if let Some(main) = rel(&files.main) {
            obj.insert("main".to_string(), serde_json::json!(main));
        }
        if let Some(module) = rel(&files.module) {
            obj.insert("module".to_string(), serde_json::json!(module));
        }
        if let Some(types) = rel(&files.types) {
            obj.insert("types".to_string(), serde_json::json!(types));
        }
    }

    // Build/merge the `exports` field.
    let mut exports_obj: serde_json::Map<String, serde_json::Value> = serde_json::Map::new();
    if !is_main {
        if let Some(existing) = obj.get("exports") {
