...
namespace utils {
	...
	/**
	 * Converts a TypeScript SourceFile into string output using TypeScript's printer.
	 * Useful for transformed ASTs where sourceFile.text no longer matches the current tree.
	 * @param sourceFile - The SourceFile to print.
	 * @param options - Optional TypeScript printer options.
	 * @returns Printed TypeScript code.
	 */
	...
	export namespace plugins {
		 ...
		 */
		export async function preProcessPluginParser(
			plugins: (SuseeTypes.SuseePlugin | SuseeTypes.SuseePluginFunction)[],
			code: string,
		...
		export async function postProcessPluginParser(
			plugins: (SuseeTypes.SuseePlugin | SuseeTypes.SuseePluginFunction)[],
			code: string,
			file?: string | undefined,
		) {
