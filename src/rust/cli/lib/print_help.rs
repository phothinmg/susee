//! Help text printer.
//!
//! Ported from `src/nodejs/cli/lib/print_help.ts`.
//!
//! The text is kept in sync with the TS help output so `susee --help` /
//! `susee -h` behaves identically across the two implementations.

/// Print the CLI help text to stdout.
///
/// Mirrors `printHelp()` from `print_help.ts`.
pub fn print_help() {
    println!(
        r#"Susee CLI.
