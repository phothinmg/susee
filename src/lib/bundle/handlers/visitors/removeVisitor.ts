import type { RequireImportObject, TypeObj } from "susee-types";
...
export const removeVisitors = {
	esmExport(context: ts.TransformationContext) {
		const { factory } = context;
		const visitor = (node: ts.Node): ts.Node => {
			// --- Case 1: Strip "export" modifiers ---
			const inside_nameSpace = checks.isInsideNamespace(node);
			if (!inside_nameSpace) {
				if (
	...
	allImports(
		...
		const visitor = (node: ts.Node): ts.Node => {
			...
			if (
				...
				if (checks.moduleType(sourceFile, file).isCommonJs) {
