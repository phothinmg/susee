...
import duplicateCallExpressionHandler from "./called.js";
import duplicateExportHandler from "./exports.js";
import duplicateImportHandler from "./imports.js";
import duplicateUpdater from "./updater.js";
...
async function duplicateRename(
  ...
  const duplicates = resolves([
    ...
    [duplicateUpdater, compilerOptions, namesMap, callNameMap],
    [
    ...
    [
      duplicateExportHandler,
      compilerOptions,
      callNameMap,
      importNameMap,
      exportNameMap,
    ],
    [duplicateImportHandler, compilerOptions, importNameMap, exportNameMap],
    [
      duplicateCallExpressionHandler,
      compilerOptions,
      callNameMap,
...
async function duplicateNotReName(
  deps: DependenciesFile[],
  compilerOptions: ts.CompilerOptions,
): Promise<DependenciesFile[]> {
  let _err = false;
  const duplicates = resolves([
    [duplicateCollector, namesMap, compilerOptions],
  ]);
...
function suseeInternalRenameDuplicate(): DepsHooks {
  return {
    type: "deps",
    async: true,
    func: async (depsFiles, compilerOptions, reName) => {
      if (reName) {
        return await duplicateRename(depsFiles, compilerOptions);
      } else {
        return await duplicateNotReName(depsFiles, compilerOptions);
      }
    },
  };
}

export default suseeInternalRenameDuplicate;
