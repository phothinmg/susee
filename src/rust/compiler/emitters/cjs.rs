...
/// let out = emit_cjs(src,None);
/// assert!(out.contains("exports.x = x;"));
/// ```
pub fn emit_cjs(source_code: &str, file_path: Option<String>) -> String {
    let entry_st = detect_source_type(source_code).unwrap();
    let mut js_code = source_code.to_string();

    if entry_st != SourceType::ts()
        && entry_st != SourceType::tsx()
        && entry_st != SourceType::mjs()
    {
    ...
    if entry_st == SourceType::ts() || entry_st == SourceType::tsx() {
        js_code = strip_types(source_code, entry_st, file_path);
    }
...
mod tests {
    ...
    #[test]
    fn emit_cjs_header() {
        let out = emit_cjs("export const x = 1;", None);
    ...
    fn emit_cjs_export_const() {
        let out = emit_cjs("export const x = 42;", None);
    ...
    fn emit_cjs_export_function() {
        let out = emit_cjs("export function foo() { return 1; }", None);
    ...
    fn emit_cjs_export_class() {
        let out = emit_cjs("export class C { m() {} }", None);
    ...
    fn emit_cjs_export_default_named_function() {
        let out = emit_cjs("export default function foo() { return 1; }", None);
        assert!(out.contains("function foo"), "got: {out}");
    ...
    fn emit_cjs_export_default_expression() {
        let out = emit_cjs("export default 42;", None);
    ...
    fn emit_cjs_export_default_anonymous_function() {
        let out = emit_cjs("export default function() { return 1; }", None);
        assert!(
    ...
    fn emit_cjs_default_import() {
        let out = emit_cjs("import x from \"foo\";\nexport { x };", None);
        assert!(
    ...
    fn emit_cjs_namespace_import() {
        let out = emit_cjs("import * as ns from \"foo\";\nexport { ns };", None);
    ...
    fn emit_cjs_named_import() {
        let out = emit_cjs("import { a } from \"foo\";\nexport { a };", None);
    ...
    fn emit_cjs_named_import_renamed() {
        let out = emit_cjs("import { a as b } from \"foo\";\nexport { b };", None);
        assert!(
    ...
    fn emit_cjs_bare_import() {
        let out = emit_cjs("import \"foo\";", None);
    ...
    fn emit_cjs_export_all() {
        let out = emit_cjs("export * from \"foo\";", None);
    ...
    fn emit_cjs_export_all_as_namespace() {
        let out = emit_cjs("export * as ns from \"foo\";", None);
        assert!(out.contains("__importStar"), "got: {out}");
    ...
    fn emit_cjs_export_from() {
        let out = emit_cjs("export { a } from \"foo\";", None);
        assert!(out.contains("__importDefault"), "got: {out}");
    ...
    fn emit_cjs_strips_types() {
        let out = emit_cjs("export const n: number = 5;", None);
        assert!(
            !out.contains(": number"),
    ...
    fn emit_cjs_destructuring_export() {
        let out = emit_cjs("export const { a, b } = { a: 1, b: 2 };", None);
        assert!(out.contains("exports.a = a;"), "got: {out}");
        assert!(out.contains("exports.b = b;"), "got: {out}");
    }
    ...
    fn emit_cjs_top_level_await_wrapped() {
        let out = emit_cjs("await foo();", None);
    ...
    fn emit_cjs_import_meta_url() {
        let out = emit_cjs("console.log(import.meta.url);", None);
    ...
    fn emit_cjs_import_meta_typescript() {
        let out = emit_cjs("const url: string = import.meta.url;", None);
        assert!(out.contains("__import_meta"), "got: {out}");
    ...
    fn emit_cjs_import_meta_in_export() {
        let out = emit_cjs("export const url = import.meta.url;", None);
        assert!(out.contains("__import_meta"), "got: {out}");
        assert!(!out.contains("import.meta"), "got: {out}");
    ...
    fn emit_cjs_import_meta_bare() {
        let out = emit_cjs("console.log(import.meta);", None);
        assert!(out.contains("__import_meta"), "got: {out}");
        assert!(!out.contains("import.meta"), "got: {out}");
    }
