...
//! `crate::core::susee_utils::apply_renames`.
...
mod tests {
    ...
    fn preserves_exports_in_entry_file() {
        let entry = DepsFile {
            ...
            bytes: 0,
            module_type: crate::core::susee_types::ModuleType::Esm,
            file_ext: crate::core::susee_types::ValidExts::Ts,
            is_jsx: false,
            is_entry: true,
        };
    ...
    fn no_changes_when_no_imports_or_exports() {
        ...
        let deps = vec![crate::core::susee_utils::make_dep("src/a.ts", src)];
        let (result, removed) = remove_handler(deps);
