...
use susee_utils::with_parsed_program;
...
pub fn check_duplicates(dep_files: Vec<DepsFile>) -> CheckReport {
...
pub fn check_anonymous(dep_files: Vec<DepsFile>) -> CheckReport {
...
pub fn check_default_exports(dep_files: Vec<DepsFile>) -> CheckReport {
...
pub fn check_missing_types(dep_files: Vec<DepsFile>) -> CheckReport {
...
fn check_statement_missing_types(
    ...
    match stmt {
        Statement::VariableDeclaration(var) => {
            // CJS require-imports (`const fs = require("node:fs")`,
            // `const { Foo } = require("./foo")`) get their type from the
            // imported module, not from an annotation — skip them entirely.
            if is_require_var(var) {
                return;
            }
            for decl in &var.declarations {
                if decl.type_annotation.is_none() {
                    if let BindingPattern::BindingIdentifier(id) = &decl.id {
                        // JS: skip if the preceding JSDoc documents this
                        // declaration. `@type`/`@typedef` directly type a
                        // variable. When the initializer is a function/arrow,
                        // `@param`/`@returns` also count as typing it.
                        let tags = if !ts_file {
                            jsdoc.and_then(|m| m.lookup(id.span.start))
                        } else {
                            None
                        };
                        let is_fn_init = matches!(
                            decl.init.as_ref().map(|b| &*b),
                            Some(Expression::FunctionExpression(_))
                                | Some(Expression::ArrowFunctionExpression(_))
                        );
                        let documented = tags.is_some_and(|t| {
                            t.has_type
                                || t.has_typedef
                                || (is_fn_init && (t.has_returns || !t.params.is_empty()))
                        });
                        if !documented {
                            ...
                            items.push(missing_type_item(
                                ...
                                line,
                                col,
                                &format!("variable \"{name}\"", name = id.name.as_str()),
                                ts_file,
                                "type",
                            ));
                        }
                ...
                // If the initializer is a function/arrow, check its return type.
                if let Some(init) = &decl.init {
                    check_function_expr_missing_return(init, dep, ts_file, jsdoc, items);
                }
            }
        }
        Statement::FunctionDeclaration(func) => {
        ...
        Statement::ExportDeclaration(exp) => match &exp.declaration {
            Declaration::VariableDeclaration(var) => {
                // CJS require-imports under `export` are also imports — skip.
                if is_require_var(var) {
                    return;
                }
                for decl in &var.declarations {
                    if decl.type_annotation.is_none() {
                        if let BindingPattern::BindingIdentifier(id) = &decl.id {
                            ...
                            if !documented {
                                let (line, col) =
                                    byte_offset_to_line_col(&dep.content, id.span.start);
                                items.push(missing_type_item(
                                    dep,
                                    line,
                                    col,
...
pub fn check_undefined_usage(dep_files: Vec<DepsFile>) -> CheckReport {
