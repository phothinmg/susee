  ...
  for (const { file, content } of deps) {
    ...
    const visit = (node: ts.Node, isGlobal = true): ts.Node | undefined => {
      ...
      if (
        ...
        if (ts.isBlock(node)) ts.visitNodes(node.statements, (n) => visit(n, false));
          ...
          if (ts.isVariableStatement(node)) {
            const newDecls = node.declarationList.declarations.map((decl) => {
              if (ts.isIdentifier(decl.name)) {
                ...
                if (fileMap && fileMap.has(fileKey)) {
            ...
            const newList = factory.updateVariableDeclarationList(node.declarationList, newDecls);
            return factory.updateVariableStatement(node, node.modifiers, newList);
        ...
        const visitor = (node: ts.Node): ts.Node => {
          if (ts.isImportDeclaration(node)) {
            ...
            if (node.importClause?.namedBindings && ts.isNamedImports(node.importClause.namedBindings)) {
              const updated = node.importClause.namedBindings.elements.map((el) => {
                const importedName = el.name.text.trim();
                const planMap = plan.get(importedName);
                if (planMap && planMap.has(spec)) {
                  const newName = planMap.get(spec) as string;
                  importMap.set(importedName, newName);
                  return factory.updateImportSpecifier(el, el.isTypeOnly, el.propertyName, factory.createIdentifier(newName));
                }
                return el;
              });
              const newNamed = factory.updateNamedImports(node.importClause.namedBindings, updated);
              const newClause = factory.updateImportClause(node.importClause, node.importClause.phaseModifier, node.importClause.name, newNamed);
              return factory.updateImportDeclaration(node, node.modifiers, newClause, node.moduleSpecifier, node.attributes);
            }
            // default import
            if (node.importClause?.name && ts.isIdentifier(node.importClause.name)) {
              ...
              if (planMap && planMap.has(spec)) {
                const newName = planMap.get(spec) as string;
                importMap.set(importedName, newName);
                const newClause = factory.updateImportClause(node.importClause, node.importClause.phaseModifier, factory.createIdentifier(newName), node.importClause.namedBindings);
                return factory.updateImportDeclaration(node, node.modifiers, newClause, node.moduleSpecifier, node.attributes);
              }
            }
            ...
            if (ts.isIdentifier(node.name)) {
              ...
              if (localMap.has(base)) {
                return factory.updateExportSpecifier(node, node.isTypeOnly, node.propertyName, factory.createIdentifier(localMap.get(base) as string));
              }
            }
            ...
            if (ts.isIdentifier(expr)) {
              ...
              if (localMap.has(base)) {
                return factory.updateExportAssignment(node, node.modifiers, factory.createIdentifier(localMap.get(base) as string));
              }
            }
            ...
            if (ts.isIdentifier(node.expression)) {
              ...
              if (replacement) {
                return factory.updateCallExpression(node, factory.createIdentifier(replacement), node.typeArguments, node.arguments);
              }
            }
            ...
            if (ts.isIdentifier(node.expression)) {
              ...
              const replacement = localMap.get(base) ?? importMap.get(base);
              if (replacement) {
                return factory.updatePropertyAccessExpression(node, factory.createIdentifier(replacement), node.name);
              }
            }
            ...
            if (ts.isIdentifier(node.expression)) {
              ...
              const replacement = localMap.get(base) ?? importMap.get(base);
              if (replacement) {
                return factory.updateNewExpression(node, factory.createIdentifier(replacement), node.typeArguments, node.arguments);
              }
            }
            ...
            if (importMap.has(name)) return factory.createIdentifier(importMap.get(name) as string);
          }
          return ts.visitEachChild(node, visitor, context);
        };
        return (root) => ts.visitNode(root, visitor) as ts.SourceFile;
