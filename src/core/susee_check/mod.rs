...
fn check(dep_files: Vec<DepsFile>) -> Result<(), ()> {
...
pub fn check_and_exit(dep_files: Vec<DepsFile>) {
    println!("{}", "susee_check: running 5 checks…".cyan().bold());
    match check(dep_files) {
        Ok(()) => {
            println!("{}", "susee_check: no issues found ✓".green().bold());
        }
        Err(()) => {
            let info = "susee_check found issues that must be fixed before bundling.";
