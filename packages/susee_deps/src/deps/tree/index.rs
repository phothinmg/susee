...
use crate::deps::checks::{run_check_duplicated, run_options_check};
use crate::deps::graph::generate_graph;
use std::path::Path;
use susee_types::{DepReturns, DependenciesTree, DepsFile, ModuleType, ProjectType, ValidExts};
use susee_utils::{detect_module_type, is_jsx_content, read_file};
//
...
pub fn susee_tree<P: AsRef<Path>>(
    ...
    let _ = run_check_duplicated(dep_files.clone());
    if check {
        run_options_check(dep_files.clone());
    }
