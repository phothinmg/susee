...
mod tests {
    ...
    fn strips_export_from_class_declaration() {
        let deps = vec![susee_utils::make_dep("src/a.ts", "export class Foo {}\n")];
        let (result, _) = remove_handler(deps);
    ...
    fn preserves_exports_in_entry_file() {
        let entry = DepsFile {
            ...
            bytes: 0,
            module_type: susee_types::ModuleType::Esm,
            file_ext: susee_types::ValidExts::Ts,
            is_jsx: false,
            is_entry: true,
        };
    ...
    fn no_changes_when_no_imports_or_exports() {
        ...
        let deps = vec![susee_utils::make_dep("src/a.ts", src)];
        let (result, removed) = remove_handler(deps);
