...
import { duplicateVisitors } from "../visitors/duplicate.js";
...
const collector = (compilerOptions: ts.CompilerOptions): BundleHandler => {
	return ({ file, content, ...rest }: DependenciesFile): DependenciesFile => {
		...
		const transformer: ts.TransformerFactory<ts.SourceFile> = (context) => {
			const visitNode = duplicateVisitors.collector(context, file, namesMap);
			/* --------------------Returns for transformer function--------------------------------- */
...
const updater = (compilerOptions: ts.CompilerOptions): BundleHandler => {
	return ({ file, content, ...rest }: DependenciesFile): DependenciesFile => {
		...
		const transformer: ts.TransformerFactory<ts.SourceFile> = (context) => {
			const visitor = duplicateVisitors.updater(context, file, namesMap);
			/* --------------------Returns for transformer function--------------------------------- */
			return (rootNode) => ts.visitNode(rootNode, visitor) as ts.SourceFile;
		}; // transformer;
		/* --------------------Returns for main handler function--------------------------------- */
...
const duplicateHandlers = {
	...
	renamed: async (
		...
		duplicateVisitors.resetDuplicateState(namesMap);
