...
use susee_utils::{UniqueName, sigil, with_parsed_program};
...
mod tests {
    ...
    fn renames_named_default_function_export() {
        let deps = vec![susee_utils::make_dep(
            "src/exp.ts",
            "export default function hello() { return 1; }\nhello();\n",
        )];
    ...
    fn export_default_renames_import_in_consumer() {
        let dep_a = susee_utils::make_dep(
            "src/exp.ts",
            "export default function hello() { return 1; }\nhello();\n",
        );
        let dep_b = susee_utils::make_dep("src/main.ts", "import myFn from \"./exp\";\nmyFn();\n");
    ...
    fn export_default_skips_entry_file() {
        let mut dep = susee_utils::make_dep(
            "src/index.ts",
            "export default function hello() { return 1; }\nhello();\n",
        );
        dep.is_entry = true;
    ...
    fn no_changes_when_no_named_default_exports() {
        ...
        let deps = vec![susee_utils::make_dep("src/normal.ts", src)];
        let result = export_default_handler(deps);
