...
import { exitWithCodeOneAndMessage } from "./test_helpers";
	...
	it("If no error", async () => {
		...
		try {
			...
			assert.deepEqual(config.entryPoints, [
				{ entry: "src/index.ts", exportPath: ".", moduleType: "both" },
			]);

			assert.deepEqual(config.postProcessHooks, []);

			assert.deepEqual(config.allowUpdatePackageJson, true);

			assert.deepEqual(config.nodeEnv, true);
		} finally {
			process.chdir(cwd);
		}
	});
