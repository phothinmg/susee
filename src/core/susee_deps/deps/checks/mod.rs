...
fn check_default(dep_files: Vec<DepsFile>) -> Result<(), ()> {
    let reports: Vec<CheckReport> = vec![
        check_duplicates(dep_files.clone()),
...
fn check_opts_default_exports(dep_files: Vec<DepsFile>) -> Result<(), ()> {
    let reports: Vec<CheckReport> = vec![check_default_exports(dep_files.clone())];
...
/// `Err(())` when at least one was reported.
fn check_opts_anonymous(dep_files: Vec<DepsFile>) -> Result<(), ()> {
    let reports: Vec<CheckReport> = vec![check_anonymous(dep_files.clone())];
...
///
/// [`susee_log::error`]: crate::core::susee_log::error
pub fn run_default_check(dep_files: Vec<DepsFile>) {
...
///
/// [`susee_log::error`]: crate::core::susee_log::error
pub fn run_check_opts_default_exports(dep_files: Vec<DepsFile>) {
...
///
/// [`susee_log::error`]: crate::core::susee_log::error
pub fn run_check_opts_anonymous(dep_files: Vec<DepsFile>) {
    println!("{}", "Susee running default_exports check…".cyan().bold());
