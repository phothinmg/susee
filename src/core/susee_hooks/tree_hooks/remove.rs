...
mod tests {
    use super::*;
    use crate::core::susee_types::{ModuleType, ValidExts};
    ...
    fn removes_default_import() {
        let deps = vec![make_dep("src/a.ts", "import bar from \"./bar\";\nbar();\n")];
        let (result, removed) = remove_handler(deps);
    ...
    fn strips_export_from_class_declaration() {
        let deps = vec![make_dep("src/a.ts", "export class Foo {}\n")];
        let (result, _) = remove_handler(deps);
    ...
    fn strips_export_from_type_alias() {
        let deps = vec![make_dep("src/a.ts", "export type Bar = string;\n")];
        let (result, _) = remove_handler(deps);
    ...
    fn removes_export_named_specifiers() {
        let deps = vec![make_dep("src/a.ts", "const foo = 1;\nexport { foo };\n")];
        let (result, _) = remove_handler(deps);
        let content = &result[0].content;
    ...
    fn strips_export_default_from_class() {
        let deps = vec![make_dep("src/a.ts", "export default class Hello {}\n")];
        let (result, _) = remove_handler(deps);
        let content = &result[0].content;
    ...
    fn no_changes_when_no_imports_or_exports() {
        ...
        let deps = vec![make_dep("src/a.ts", src)];
        let (result, removed) = remove_handler(deps);
