...
/**
 * Bundles all configured entry points using the project configuration.
 *
 * The function:
 * - Loads the bundling configuration.
 * - Builds each entry point in the requested module format (ESM, CommonJS, or both).
 * - Writes package metadata if configured to do so.
 * - Logs progress for each entry point.
 *
 * @returns A promise that resolves when all entry points are bundled.
 */
